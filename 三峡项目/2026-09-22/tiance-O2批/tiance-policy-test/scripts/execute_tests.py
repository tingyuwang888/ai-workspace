...
def query_component_log(host, token, cookie, csrf):
    ...
    返回 (status_code, response_dict)。
    """
    url = (f"{host.rstrip('/')}/noahApi/policy/report/getAllCompontlog"
           f"?id={token}&reportAuth=false&tokenId={token}&type=1")
    headers = {"Accept": "application/json"}
    if cookie:
        headers["Cookie"] = cookie
    if csrf:
...
def parse_component_evidence(comp_log_resp, rule_catalog=None):
    """Extract structured rule, function, and third-party evidence."""
...
def main():
    ...
    execute_tests(
        ...
        rule_catalog=config.get("testExecution", {}).get("ruleCatalog", []),
        run_id=run_id,
