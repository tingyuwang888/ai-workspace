...
| `decisionTool.rows` / `decisionTool.axes`+`cells` | 决策表按行、决策矩阵按单元格的条件与赋值 |
...
> **引用决策工具的策略**（策略内嵌或调用决策表/决策矩阵/决策树，或输入是决策工具导出文件）：
> 改走 [decision-tool-testing.md](references/decision-tool-testing.md) 全链路——导出的 `base64(gzip(JSON))`
> 用 `decode_pls.py` 解码取 `content`/`axes+cells`/`defaultDecisions` → 反推成策略落地文档（决策表行/矩阵单元格
> → 规则行，字段→字段映射，枚举→风险决策类型） → 按每行/每单元格/每条路径的边界设计用例 → 真实执行后从
> `executeDetail` 取命中路径作证据 → 五态报告。未上线的决策工具无导出入口，改走 profile `?type=view` 只读态读模型。
...
- [decision-tool-testing.md](references/decision-tool-testing.md) — 引用决策工具（决策表/决策矩阵/决策树）的策略：导出解码 → 反推落地文档 → 每行/单元格/路径边界用例 → `executeDetail` 命中路径证据 → 五态报告；未上线工具走 profile `?type=view` 只读；配套解码器 `scripts/decode_pls.py`
