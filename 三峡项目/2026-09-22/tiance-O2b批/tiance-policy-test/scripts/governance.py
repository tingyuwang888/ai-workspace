...
def aggregate_acquiring(results, run_id, design=None):
    ...
    from acquiring_runner import (  # lazy: a runner import defect must not break governance itself
        environment_regression_cases, make_case_key, validate_acq_design)
    ...
    # O2b: coverageGaps stays [] until a checkpoint design contract is supplied;
    # then a branch is a gap when none of its caseKeys landed a 通过/失败 case,
    # mirroring the platform classify() gap semantics.
    gaps = []
    if design is not None:
        executed = {case["caseKey"] for case in cases if case["status"] in {"通过", "失败"}}
        gaps = [copy.deepcopy(branch) for branch in design["coveragePlan"]
                if not executed.intersection(branch["caseKeys"])]
...
def main():
    ...
    aggregate.add_argument("--design", default=None,
                           help="收单链设计合同 JSON (O2b)：据此计算 coverageGaps；不传则 coverageGaps 保持 []")
    ...
    try:
        if args.command == "validate-design":
            ...
            report = aggregate_acquiring([read(path) for path in files],
                                         args.run_id or f"acq-{root.name}", design=design)
            ...
            print(json.dumps({"counts": report["counts"], "fullSuccess": report["fullSuccess"],
                              ...
                              "coverageGaps": len(report["coverageGaps"]),
