...
class TestCheckpointDesignGate(unittest.TestCase):
    CK = TestDryRunEndToEnd.CK

    def design_for(self, ck, **overrides):
        case = {"caseKey": ar.make_case_key(ck["row"], ck["rule_code"]),
                "row": ck["row"], "ruleCode": ck["rule_code"],
                "targetRuleSet": ck["target_rule_set_code"], "caseType": ck["case_type"],
                "designReview": {"status": "approved", "evidenceRefs": ["row47.md"]}}
        case.update(overrides)
        return {"schemaVersion": "1.0", "runId": "acq-designGate", "cases": [case],
                "coveragePlan": [{"ruleCode": case["ruleCode"],
                                  "targetRuleSet": case["targetRuleSet"],
                                  "branchId": "b-main", "caseKeys": [case["caseKey"]]}]}

    def _run(self, tmp, design=None):
        ck = tmp / "ck.json"
        ck.write_text(json.dumps(self.CK, ensure_ascii=False), encoding="utf-8")
        argv = ["--checkpoint", str(ck), "--output", str(tmp / "run"),
                "--registry", str(tmp / "reg.json"), "--dry-run"]
        if design is not None:
            dpath = tmp / "design.json"
            dpath.write_text(json.dumps(design, ensure_ascii=False), encoding="utf-8")
            argv += ["--design", str(dpath)]
        self.assertEqual(ar.main(argv), 0)
        summary = json.loads((tmp / "run" / "runner_summary.json").read_text(encoding="utf-8"))
        return summary["cases"][0]

    def test_matching_design_lets_checkpoint_run(self):
        with tempfile.TemporaryDirectory() as tmp:
            case = self._run(pathlib.Path(tmp), self.design_for(self.CK))
            self.assertNotEqual(case["status"], "无效用例")
            self.assertTrue(case["namespace"].startswith("QW_"))

    def test_rule_code_mismatch_marks_invalid(self):
        with tempfile.TemporaryDirectory() as tmp:
            design = self.design_for(self.CK, ruleCode="ACQ_OTHER_000001")
            # keep caseKey consistent so only the ruleCode comparison fails
            case = self._run(pathlib.Path(tmp), design)
            self.assertEqual(case["status"], "无效用例")
            self.assertIn("checkpoint_design_mismatch", case["reason_codes"])
            self.assertIsNone(case["namespace"])

    def test_checkpoint_absent_from_design_is_invalid(self):
        with tempfile.TemporaryDirectory() as tmp:
            design = self.design_for(self.CK)
            design["cases"][0]["caseKey"] = "acq-999-ACQ_NOT_THIS"
            design["cases"][0]["row"] = 999
            design["coveragePlan"][0]["caseKeys"] = ["acq-999-ACQ_NOT_THIS"]
            case = self._run(pathlib.Path(tmp), design)
            self.assertEqual(case["status"], "无效用例")
            self.assertIn("checkpoint_design_mismatch", case["reason_codes"])

    def test_no_design_leaves_behavior_unchanged(self):
        with tempfile.TemporaryDirectory() as tmp:
            case = self._run(pathlib.Path(tmp), design=None)
            self.assertNotEqual(case["status"], "无效用例")
            self.assertTrue(case["namespace"].startswith("QW_"))
