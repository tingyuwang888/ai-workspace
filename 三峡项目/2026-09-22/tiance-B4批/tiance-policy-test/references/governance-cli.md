...
The acquiring chain (acquiring_runner.py, direct acquiring HTTP) is a separate
...
### Acquiring-chain checkpoint design gate (O2b)

The acquiring chain now has a run-level design contract that mirrors the
platform chain's design-manifest. It is a single JSON file validated against
`$defs/acqDesign` in [governance.schema.json](governance.schema.json):

```json
{
  "schemaVersion": "1.0",
  "runId": "acq-20260922-a",
  "cases": [{
    "caseKey": "acq-047-ACQ_RS3_000014",
    "row": 47,
    "ruleCode": "ACQ_RS3_000014",
    "targetRuleSet": "rs_acq_unauthorized",
    "caseType": "正案例",
    "designReview": {"status": "approved", "evidenceRefs": ["review.json#/47"]}
  }],
  "coveragePlan": [{
    "ruleCode": "ACQ_RS3_000014",
    "targetRuleSet": "rs_acq_unauthorized",
    "branchId": "positive",
    "caseKeys": ["acq-047-ACQ_RS3_000014"]
  }]
}
```

Each `caseKey` follows the same `acq-{row:03d}-{rule_code}` formula the
aggregate uses, so a design case binds to exactly one executed archive. The
gate is opt-in on both ends and byte-identical to prior behaviour when omitted:

- `acquiring_runner.py --design /path/acq-design.json`: before a checkpoint is
  run, its `row`/`rule_code`/`case_type`/`target_rule_set_code` are validated
  against `$defs/acqCheckpoint` and matched to an approved design case by
  `caseKey`. A checkpoint absent from the design, or whose contract contradicts
  it, is judged `无效用例` with reasonCode `checkpoint_design_mismatch` and no
  namespace token is allocated.
- `governance.py aggregate-acquiring --design /path/acq-design.json`:
  `coverageGaps` is computed from the plan — a `coveragePlan` branch is a gap
  when none of its `caseKeys` landed a 通过/失败 case, exactly mirroring the
  platform `classify()` gap semantics. When present it also gates
  `fullSuccess` (all cases 通过 AND no gaps).

Without `--design`, `coverageGaps` stays `[]` and no checkpoint is blocked, so
the existing baseline is unchanged. The design file is validated on both load
paths; `validate_acq_design` rejects duplicate caseKeys, duplicate branches and
branches referencing unknown caseKeys. Like the platform gate, this proves
schema and identity binding only — the agent still establishes rule semantics,
history/fixture readiness and review evidence before declaring `approved`.
