...
Two findings surfaced during B4 verification (NOT fixed here — need their own sign-off):
1. Publish-subset completeness: the staging scripts/ folder omits result_evaluator.py,
   which governance.py imports, so the subset's governance-dependent tests error at
   collection (ModuleNotFoundError). test_desensitized.py runs standalone and passes,
   but a team-repo push must ship the full generic script set (result_evaluator + others)
   or the shipped package cannot self-test.
