...
> **Source of truth**: The machine-readable profile
> [`hlb_acquiring_profile.json`](hlb_acquiring_profile.json) is the canonical,
> single source for endpoint addresses, request encodings, required field groups,
> and enum values. `scripts/validate-test-data.py` loads `ENDPOINTS`/`ENUMS`
> directly from that JSON at import time — this Markdown is the human-facing view
> only and must stay in sync with it (guarded by `scripts/test_hlb_profile.py`).
> When a value here disagrees with the JSON, the JSON wins; fix the JSON and let
> the guard test flag the drift.
...
| AcqDecline | http://<HLB_HOST>:8000/riskService/trade/riskDecision/acqDecline | application/json |
...
**Important**:
- AcqAuthorization and AcqDecline use JSON encoding
...
Uses JSON encoding (like AcqAuthorization).
