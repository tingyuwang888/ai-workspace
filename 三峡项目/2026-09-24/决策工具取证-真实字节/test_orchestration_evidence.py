...
- 决策工具树 / 矩阵 / 表：**真实字节** —— 逐字来自 2026-09-24 在三峡 ``sxdb`` 实盘重抓的
  ...
  出参体现。
- 网关 / 子策略：documented-only —— 载荷形状仅来自 ``api-reference.md``，为合成 fixture，
  只证明宽容解析与降级不失败，**不**主张平台一定如此吐数。
"""
...
def _gateway_response():
    """ExclusiveGateway（documented-only 合成）：判定入参，路由出参。"""
    gw = {
        "nodeType": "ExclusiveGateway",
        "nodeName": "路由网关",
        "nodeId": "gw-1",
        "nodeInputList": [{"fieldName": "S_E_APIMODEL", "value": "30"}],
        "nodeOutputList": [{"fieldName": "routeTarget", "value": "民营分支"}],
        "extension": {"routeCondition": "S_E_APIMODEL == 30"},
    }
    flow = [
        {"nodeType": "StartEvent", "nodeName": "开始", "nodeId": "s"},
        gw,
    ...
    return _wrap(flow, diagram_line=["l1", "l2"], token="gw-token")


def _subpolicy_response():
    """SubPolicyNode + CollectionSubPolicyNode（documented-only 合成）：挖子 token。"""
    sp = {
        "nodeType": "SubPolicyNode",
        "nodeName": "普通子策略",
        "nodeId": "sp-1",
        "extension": {"subPolicyResultList": [
            {"token": "child-token-a", "policyCode": "SUB_POLICY_X"},
        ]},
    }
    csp = {
        "nodeType": "CollectionSubPolicyNode",
        "nodeName": "集合循环子策略",
        "nodeId": "csp-1",
        "nodeOutputList": [{"serviceParam": "children", "value": json.dumps([
            {"tokenId": "child-token-b", "policyCode": "SUB_POLICY_Y"},
            {"tokenId": "child-token-c", "policyCode": "SUB_POLICY_Z"},
        ])}],
    }
    flow = [
        {"nodeType": "StartEvent", "nodeName": "开始", "nodeId": "s"},
    ...
    return _wrap(flow, diagram_line=["l1", "l2", "l3"], token="sp-token")
...
class TestEndToEndContract(unittest.TestCase):
    ...
    def test_gateway_parsed_and_flow_visible(self):
        ...
        self.assertEqual([("S_E_APIMODEL", "30")],
                         [(i["field"], i["value"]) for i in gw["inputs"]])
        self.assertEqual([("routeTarget", "民营分支")],
                         [(r["field"], r["value"]) for r in gw["routeFields"]])
    ...
    def test_subpolicy_child_tokens_extracted_from_both_containers(self):
        ...
        by_name = {node["nodeName"]: node for node in ev["subPolicies"]}
        self.assertIn("普通子策略", by_name)
        self.assertIn("集合循环子策略", by_name)
        normal_children = {c["policyCode"] for c in by_name["普通子策略"]["children"]}
        coll_children = {c["policyCode"] for c in by_name["集合循环子策略"]["children"]}
        self.assertEqual({"SUB_POLICY_X"}, normal_children)
        # 集合循环子策略的子 token 藏在 nodeOutputList 的 JSON 字符串里
        self.assertEqual({"SUB_POLICY_Y", "SUB_POLICY_Z"}, coll_children)

    def test_subpolicy_present_but_unavailable_is_missing_not_mismatch(self):
        # documented-only 形状：子项无 evidenceStatus → 默认 unavailable → 只缺不失败
...
class TestEndToEndRender(unittest.TestCase):
    ...
    def test_gateway_render_shows_route(self):
        ...
        self.assertIn("网关: 路由网关(ExclusiveGateway)", text)
        self.assertIn("判定 S_E_APIMODEL=30", text)
        self.assertIn("路由 routeTarget=民营分支", text)
    ...
    def test_subpolicy_render_lists_children_status(self):
        ...
        self.assertIn("子策略: 普通子策略", text)
        self.assertIn("子项 SUB_POLICY_X", text)
