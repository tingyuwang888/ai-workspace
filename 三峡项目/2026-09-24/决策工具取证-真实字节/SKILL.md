...
不套用通用套的外部收单端点/枚举，也不把天策 policyTest 提交当成历史事件入库接口。
HLB 收单链的端点/枚举/必填字段以机读文件
[references/hlb_acquiring_profile.json](references/hlb_acquiring_profile.json) 为单一来源
（`scripts/validate-test-data.py` 在导入时据此派生 `ENDPOINTS`/`ENUMS`），
人读视图为 [references/acquiring-hlb-profile.md](references/acquiring-hlb-profile.md)，
一致性由 `scripts/test_hlb_profile.py` 守卫。二者均含客户标识，仅存在于本地技能与私仓归档，
不进入通用发布子集。
...
runner 归档，不重判）生成同构 `governed-results.json`。收单链另有可选的 checkpoint
设计门禁（`--design`，O2b）：runner 侧据设计合同把未批准/不匹配的 checkpoint 判为
无效用例，aggregate 侧据此计算 `coverageGaps`；不传 `--design` 时行为与既有基线逐字节一致。
...
| `decisionTool` | 被测策略引用的决策工具（决策表/矩阵/树）模型，仅在需要时填 |
| `decisionTool.modelType` | 决策工具类型：`D_TABLE` / `D_MATRIX` / `D_TREE` |
| `decisionTool.inputs` / `decisionTool.outputs` | 决策工具的入参/出参字段（含数据类型） |
| `decisionTool.rows` / `decisionTool.axes`+`cells` / `decisionTool.paths` | 决策表按行、决策矩阵按单元格、决策树按根→叶路径的条件与赋值 |
| `decisionTool.defaultDecision` | 全部条件不命中时的兜底决策 |
...
复杂编排用例可额外声明以下四类 `expectedEvidence`，执行器逐项消费。决策工具的
`executeDetail` 证据形态已在三峡 `sxdb` 逐字重抓的只读实验室运行（2026-09-24）中实测验证：
树 / 表为 `ROOT→CHILD→DECISION`（CHILD 携带命中 `conditionSet`、只记录命中分支），矩阵为
`ROOT→DECISION`（无 CHILD 步、`nodeId` 为空串，命中仅由 DECISION 出参体现）；子策略与网关的解析仅有文档依据、
未经平台采集，一律按“缺失即 `inconclusive`”降级，绝不据此判定命中或未命中。
...
```json
{
  "expectedEvidence": {
    "decisionTools": [
      {
        "toolCode": "TREE2609231662259291",
        "fieldValues": {"C_F_LEVEL": "1"},
        "hitConditions": [
          {"field": "C_F_SALARY", "operator": "<=", "expected": 100, "actual": 90}
        ]
      }
    ],
    "pathContains": ["开始", "还款决策树", "结束"],
    "nodeVisited": [{"nodeId": "t2"}],
    "subPolicies": [{"policyCode": "SUB_A", "expectedStatus": "hit"}]
  },
  "executionPath": {"order": "strict"}
}
```

`decisionTools` 按 `nodeName`/`nodeId`/`toolCode` 别名定位命中的决策工具节点，校验出参
字段值（`fieldValues`，含字符串/数字归一）与命中条件（`hitConditions`）。同工具被复用
时 `nodeName` 相同、无判别力，必须用 `nodeId` 或 `toolCode` 定位，否则记为“身份不唯一”
而非误判；执行详情缺失或解析失败时只记 `inconclusive`，不断言分支被跳过。`pathContains`
声明执行路径必须出现的节点名，配合 `executionPath.order="strict"` 时按流顺序严格校验。
`nodeVisited` 校验节点被访问，重名场景用 `nodeId` 去歧义。`subPolicies` 声明集合循环子
策略的子 token；子策略证据不可用时保持 `inconclusive`，不计入断言失败。
...
> **引用决策工具的策略**（策略内嵌或调用决策表/决策矩阵/决策树）：默认已有落地文档，改走
> [decision-tool-testing.md](references/decision-tool-testing.md)——按工具类型把分支铺成用例（决策表行 /
> 矩阵单元格 / 决策树根→叶路径 + 阈值±ε 边界，树默认分支可测、表 else 常不可达）→ 在画布搭建/核对引用策略
> （画布路由、草稿 vs 已发布 getDetail、保存/上线接口区别）→ 可编程跑单笔测试（`lab/policytest/create` →
> `baseInfo` 预热 → `getAllCompontlog`，终值看 `contextFields`、逐节点命中看 `fieldMap.nodeIdList`）→ 五态报告。
> 未上线的工具走 profile `?type=view` 只读态读模型。决策树 D_TREE schema 已锁定。

> **复杂编排策略**（一条流串多个决策工具/规则集/函数节点，或带网关分支、集合循环/普通子策略）：
...
> `subPolicies`）→ 降级不失败（证据缺失=`inconclusive`、值不符=`failed`；子策略/网关未经实测只降级不判失败）。
> 决策工具证据形状已三峡 `sxdb` 三次只读实测；网关、子策略载荷仅文档级、解析宽容实现。
...
- [orchestration-testing.md](references/orchestration-testing.md) — 复杂编排策略（多节点链/网关/集合循环子策略）：`getAllCompontlog` 逐节点取证（`flowModelinAndOutputParams` 序为权威执行顺序）→ 四类 `expectedEvidence` 断言（`decisionTools`/`pathContains`/`nodeVisited`/`subPolicies`）→ 降级不失败；决策工具证据已三峡 `sxdb` 实测，网关/子策略仅文档级
