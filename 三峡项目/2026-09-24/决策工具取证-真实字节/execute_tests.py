...
_CHILD_TOKEN_KEYS = ("token", "tokenId", "childToken", "subToken")
...
def _extract_child_tokens(node):
    """Best-effort child token extraction for SubPolicyNode.

    CAVEAT: SubPolicyNode is documented in api-reference.md (``runData
    nodeType=8`` returns child strategy tokens) but its payload shape has not
    been captured from a real platform run, so this walks every known container
    instead of assuming one layout. Callers must treat an empty result as
...
def _parse_gateway_node(node):
    ...
    CAVEAT: documented node type, payload shape not yet captured from a real
    platform run; extraction is deliberately permissive. Only the generic flow
    node containers (``nodeInputList`` / ``nodeOutputList``) are interpreted —
    the former as the values the gateway evaluated, the latter as what it routed
    on — everything else is passed through as ``extension`` for manual reading.
    """
...
# runs; SubPolicyNode / gateways come from api-reference.md and are unverified,
# so their parsing is permissive and callers must treat missing entries as
# "evidence unavailable" rather than "node did not run".
DECISION_TOOL_NODE_TYPES = ("DecisionToolServiceNode",)
SUB_POLICY_NODE_TYPES = ("SubPolicyNode", "CollectionSubPolicyNode")
