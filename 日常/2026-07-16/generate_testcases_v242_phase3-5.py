...
def generate_third_party_exception_cases(data, fields_dict):
    """为策略中每个三方接口生成 3 条异常用例（超时/空返回/异常值）。"""
    ...
    def next_id():
        n = counter[0]
        counter[0] += 1
        return f"TP_EXC_{n:03d}"

    common_params = {
        "S_S_BIZID": "BIZ_TP_EXCEPTION_TEST",
        "S_S_CUSTNO": "CUST_TP_EXCEPTION",
        "S_S_ORGCODE": "ORG_DEFAULT",
    ...
    for iface in tp_interfaces:
        ...
        cases.append({
            "id": next_id(),
            "desc": f"三方接口异常-超时: {iface_desc}",
            "group": "三方接口异常",
            "caseType": "异常案例",
            "scenario": f"调用{iface_desc}超时或返回错误码，验证策略容错处理",
            "expected": "策略不因接口异常崩溃，走降级逻辑或返回默认值",
            ...
            "thirdPartyMock": {
                ...
                "fields": [],
                "mockHint": f"mock {iface_desc} 超时或返回错误（如 status=timeout, errCode=500）",
        ...
        cases.append({
            "id": next_id(),
            "desc": f"三方接口异常-空返回: {iface_desc}",
            "group": "三方接口异常",
            "caseType": "异常案例",
            "scenario": f"调用{iface_desc}返回空数据集（空JSON/null），验证策略空值处理",
            "expected": "策略不因空数据崩溃，相关字段置默认值或跳过计算",
            "params": dict(common_params),
            "targetRuleSet": "",
            "targetRule": "",
            "thirdPartyMock": {
                "interfaces": [iface_name],
                "fields": [],
                "mockHint": f"mock {iface_desc} 返回空数据（data=null 或 空数组）",
                "exceptionType": "emptyResponse",
            },
        })

        # 3. 接口返回异常值（极值/负数/非预期类型）
        cases.append({
            "id": next_id(),
            "desc": f"三方接口异常-异常值: {iface_desc}",
            "group": "三方接口异常",
            "caseType": "异常案例",
            "scenario": f"调用{iface_desc}返回极端值（负数/超大值/非预期类型），验证策略边界处理",
            "expected": "策略不因异常值产生错误决策，触发边界保护或默认值兜底",
            "params": dict(common_params),
            "targetRuleSet": "",
            "targetRule": "",
            "thirdPartyMock": {
                "interfaces": [iface_name],
                "fields": [],
                "mockHint": f"mock {iface_desc} 返回异常值（如金额=-1, 数量=999999999, 字段类型错误）",
                "exceptionType": "abnormalValue",
            },
        })

    if cases:
        print(f"[三方接口异常] 为 {len(tp_interfaces)} 个接口生成 {len(cases)} 条异常用例",
              file=sys.stderr)
    return cases
...
def compute_coverage_pre_report(all_cases, rules, rule_sets, functions, fields_dict, data):
    ...
    # --- 1. 规则覆盖率 ---
    rule_cases = [c for c in all_cases if c.get("group") == "规则"]
    rule_detail = {}
    for r in rules:
        ...
        has_boundary = any("边界" in c.get("caseType", "") or "边界" in c.get("desc", "") for c in rc)
    ...
    fully_covered = sum(1 for v in rule_detail.values() if v["cases"] >= 3)
    ...
    for rs in non_alert_rs:
        ...
        rs_cases = [c for c in all_cases if c.get("targetRuleSet", "").find(rs_code) >= 0]
    ...
    # --- 3. 函数覆盖率 ---
    func_cases = [c for c in all_cases if c.get("group") == "函数"]
    func_detail = {}
    for f in functions:
        fname = f.get("name", "")
        fc = [c for c in func_cases if fname in c.get("scenario", "") or fname in c.get("desc", "")]
        func_detail[fname] = {
            "type": f.get("type", ""), "cases": len(fc),
            "hasPositive": any(c.get("caseType") == "正案例" for c in fc),
            "hasNegative": any(c.get("caseType") == "反案例" for c in fc),
        }
    ...
    used_fields = set()
    for c in all_cases:
        used_fields.update(c.get("params", {}).keys())
    # 排除非策略字段（通用参数）
    ...
    report["dimensions"]["fields"] = {
        ...
        "usedInTestCases": len(strategy_used),
    ...
    # --- 6. 交叉矩阵覆盖率 ---
    cross_cases = [c for c in all_cases if c.get("group") == "规则集联动矩阵"]
    ...
    # --- 7. 决策流分支覆盖率 ---
    flow_cases = [c for c in all_cases if c.get("group") == "决策流分支覆盖"]
    combined_cases = [c for c in all_cases if c.get("group") == "跨子策略综合"]
...
def generate_all(parsed_json_path, output_path, feedback_path=None):
    ...
    # v2.4.2 Phase 3-4: 三方接口异常用例
    tp_exception_cases = generate_third_party_exception_cases(data, data["fields"])

    all_cases = rule_cases + func_cases + flow_cases + alert_cases + combined_cases + cross_cases + region_cases + tp_exception_cases
