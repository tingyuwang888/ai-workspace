#!/usr/bin/env python3
"""
tiance-agent-loop Orchestrator v1.1.0
多轮自动编排：触发 → 生成 → 执行 → 校验 → 反馈 → 收敛判定
...
def step2b_execute(iter_dir, testcases_path, report_path=None,
    ...
    if host and cookie and strategy_config:
        ...
        if ok and results_json.exists():
            ...
            # 注意：execute_tests.py 输出 results.json（非 Excel 报告）
            # 后续 Step 3 的 checker 需要 Excel 报告，这里需要标记为需要转换
            # 暂时将 results.json 作为报告占位，Step 3 需兼容 JSON 输入
            # 或者调用 update_report.py 将 results.json 合并到 testcases.xlsx
            testcases_xlsx = iter_dir / 'testcases.xlsx'
            if testcases_xlsx.exists():
                shutil.copy2(str(testcases_xlsx), str(test_report))
                log(f'使用用例 Excel 作为报告模板: {test_report.name}')
            return True, str(test_report)
        else:
            log('API 执行失败', 'ERROR')
            if stderr:
