...
# ---------------------------------------------------------------------------
# 反馈注入 (v2.4.0: --feedback 支持，闭合 Agent Loop)
# ---------------------------------------------------------------------------
...
def generate_all(parsed_json_path, output_path, feedback_path=None):
    ...
    all_cases = rule_cases + func_cases + flow_cases + alert_cases + combined_cases

    # v2.4.0: 注入上轮 Agent Loop 反馈（fixParams / adjustExpected / removeCases）
    if feedback_path:
        fb_actions = load_feedback(feedback_path)
        if fb_actions:
            fb_applied = apply_feedback(all_cases, fb_actions)
            print(f"已应用 {fb_applied}/{len(fb_actions)} 条反馈动作 (v2.4.0)", file=sys.stderr)
    ...
    print(f"生成完成！共 {len(all_cases)} 条测试用例 (v2.4.0)", file=sys.stderr)
    for mod, cnt in by_module.items():
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="天策策略测试用例生成引擎 v2.4.0",
