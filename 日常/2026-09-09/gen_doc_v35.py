...
OUT = "/Users/td/.qoderwork/workspace/mttrqawk3qp6mzcs/outputs/天策系统策略测试组件覆盖能力说明_V3.5.docx"
...
add_fig(FIG1, "图1 策略测试单笔执行链路（指标按三段式验证）")
add_table([
...
add_fig(FIG2, "图2 策略测试治理闭环（A-H 八阶段）")
...
doc.add_heading("用例生成 Skill  tiance-testcase-generator", level=2)
...
doc.add_heading("平台执行 Skill  tiance-policy-test", level=2)
...
doc.add_heading("报告审计 Skill  tiance-report-checker", level=2)
...
doc.add_heading("闭环编排 Skill  tiance-agent-loop", level=2)
...
para("参考资料  当前能力概览；天策策略测试组件覆盖能力说明 V3.3；tiance-* 四测试 Skill 治理合同（governance.py / risk-governance.md）。")
