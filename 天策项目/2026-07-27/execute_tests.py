    ...
    for i, tc in enumerate(test_cases):
        ...
        if tc_id in completed_ids:
            ...
            if skipped <= 3 or skipped % 50 == 0:
                print(f"[{i+1}/{total}] {tc_id}: 跳过（已完成）", file=sys.stderr)
            continue
        ...
        for retry in range(max_retry + 1):
            try:
                ...
                if executed_count % 10 == 0 or i == total - 1:
                    _passed = sum(1 for r in results if r["pass"])
                    _failed = executed_count - _passed
                    print(f"progress: {i+1}/{total} passed={_passed} failed={_failed}",
                          file=sys.stderr)
                break
    ...
    if args.resume and Path(args.output).exists():
        try:
            ...
            if isinstance(existing_results, list):
                ...
                print(f"[resume] 加载已有结果: {len(existing_results)} 条, "
                      f"跳过已完成: {len(completed_ids)} 条", file=sys.stderr)
            else:
                existing_results = None
        except (json.JSONDecodeError, KeyError) as e:
            print(f"[resume] 已有结果文件损坏，重新开始: {e}", file=sys.stderr)
            existing_results = None
